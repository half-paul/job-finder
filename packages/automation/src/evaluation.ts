...
import { digest } from "@jobfinder/shared/hash";
...
function apiKey() {
  ...
  if (!value) throw new AppError(503, "AI matching is not configured.");
  return value;
}
...
async function withEvaluationLock<T>(
  userId: string,
  action: (db: AutomationDb) => Promise<T>,
) {
...
export async function evaluateJobWithDb(
  userId: string,
  jobId: string,
  db: AutomationDb,
) {
  ...
  if (!job) throw new AppError(404, "Opportunity not found.");
  ...
  const ai = await createStructuredEvaluation(
    ...
    { apiKey: apiKey(), model: explanationModel },
  );
