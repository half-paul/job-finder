...
Being undocumented is not the same as being unreachable: `compose.yaml`'s `crawler` service still keeps `env_file: .env.local`, the one file the README tells every operator to create, so an operator's own file could still set any of these three variables even without knowing they exist. The actual barrier is `NODE_ENV`: `compose.yaml` sets `NODE_ENV: production` explicitly in the crawler service's `environment:` block, which Compose gives precedence over `env_file`, and all three insecure gates (`insecureTestHostnameAllowed` in `policy.ts`, the `ignoreHTTPSErrors` check in `session.ts`, and `NODE_TLS_REJECT_UNAUTHORIZED` being stripped from the process environment at startup in `index.ts` when `NODE_ENV === "production"`, since that variable is read by Node's own TLS internals rather than by any call site of ours) refuse outright on that value, before the underlying env var is even read. `compose.e2e.yaml` overrides `NODE_ENV` back to a non-production value for the crawler service only, which is why that overlay — and only that overlay — can make the three insecure vars take effect.
