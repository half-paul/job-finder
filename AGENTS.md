...
- Keep provider, matching, automation and shared contracts independent of Next.js so `apps/worker` reuses them unchanged. Import hashing from `@jobfinder/shared/hash`, never from the browser-facing shared entry point.
...
- Worker paths get both: pure schedule/threshold/dedupe assertions in `tests/unit`, and a real-database test that calls the handler with an injected `fetchImpl` so no test ever waits on a live provider. Never schedule a scan against a real board in a default test run.
...
- Scheduled evaluation inherits every rule above, including the budget check. The digest narrative is the only other model call in Phase 4; digest counts, ordering and highlights are computed deterministically, and an unavailable model falls back to deterministic text.
...
- Alerts and digests are in-app records only. Do not add email, SMS, push, Slack or Telegram delivery, and do not describe those channels as present. Alerts are opt-in, keyed by a deterministic dedupe key, and never created for a job that was not evaluated.
